...
namespace JBooth.MicroSplat
{
    ...
    public class MicroSplatBaseFeatures : FeatureDescriptor
    {
        ...
        // files to include
        private static TextAsset properties_splat;

        // Can we template these somehow?
        private static readonly Dictionary<DefineFeature, string> sFeatureNames =
            new Dictionary<DefineFeature, string>();

        private static readonly GUIContent CAlbedoTex = new GUIContent("Albedo/Height Array",
            "Texture Array which contains albedo and height information");

        private static readonly GUIContent CNormalSpec = new GUIContent("Normal/Smooth/AO Array",
            "Texture Array with normal, smoothness, and ambient occlusion");

        private static readonly GUIContent CNormal = new GUIContent("Normal Array", "Texture Array with normals");

        private static readonly GUIContent CEmisMetal =
            new GUIContent("Emissive/Metal Array", "Texture Array with emissive and metalic values");

        private static readonly GUIContent CSmoothAO =
            new GUIContent("Smoothness/AO Array", "Texture Array with Smoothness and AO");

        private static readonly GUIContent CPerTexUV = new GUIContent("UV Scale",
            "UV Scale for the texture. You may need to change your sampler settings if this is enabled.");

        private static readonly GUIContent CPerTexUVOffset = new GUIContent("UV Offset", "UV Offset for each texture");

        private static readonly GUIContent CPerTexInterp =
            new GUIContent("Interpolation Contrast", "Control blend of sharpness vs other textures");

        private static readonly GUIContent CPerTexTint = new GUIContent("Tint", "Tint color for albedo");

        private static readonly GUIContent CPerTexNormStr =
            new GUIContent("Normal Strength", "Increase or decrease strength of normal mapping");

        private static readonly GUIContent CPerTexAOStr =
            new GUIContent("AO Strength", "Increase or decrease strength of the AO map");

        private static readonly GUIContent CPerTexSmoothStr =
            new GUIContent("Smoothness Strength", "Increase or decrease strength of the smoothness");

        private static readonly GUIContent CPerTexMetallic =
            new GUIContent("Metallic", "Set the metallic value of the texture");

        private static readonly GUIContent CPerTexBrightness = new GUIContent("Brightness", "Brightness of texture");
        private static readonly GUIContent CPerTexContrast = new GUIContent("Contrast", "Contrast of texture");

        private static readonly GUIContent CPerTexHeightOffset = new GUIContent("Height Offset",
            "Allows you to adjust the heightfield of each texture up or down");

        private static readonly GUIContent CPerTexHeightContrast =
            new GUIContent("Height Contrast", "Allows you to adjust the contrast of the height map");

        private readonly GUIContent CDisableHeightBlend = new GUIContent("Disable Height Blending",
            "Disables height based blending, which can be a minor speed boost on low end platforms");

        private readonly GUIContent CEmissiveArray =
            new GUIContent("Emissive/Metallic Array", "Sample an emissive and metallic texture array");

        private readonly GUIContent CForceShaderModel = new GUIContent("Shader Model",
            "Force a specific shader model to be used. By default, MicroSplat will use the minimum required shader model based on your shader settings");


        private readonly GUIContent CInterpContrast = new GUIContent("Interpolation Contrast",
            "Controls how much hight map based blending is used");

        private readonly GUIContent CLightingMode = new GUIContent("Lighting Model",
            "Override Unity's automatic selection of a BDRF function to a fixed one. This will force the shader to render in forward rendering mode when not set to automatic");

        private readonly GUIContent CMaxTexCount = new GUIContent("Max Texture Count",
            "How many textures your terrain is allowed to use - if you are using less than 13 textures, this allows you to optimize our the work of sampling the extra control textures, and saves samplers");

        private readonly GUIContent CPackMode = new GUIContent("Packing Mode",
            "Mode in which the textures are packed (2 arrays for better speed, 3 arrays for better quality)");

        private readonly GUIContent CSamplerMode = new GUIContent("Sampler Mode",
            "Force usage of manual mip selection in the shader (fast) or gradient samplers (slow). Mostly only used when PerTexture UV Scale is used. See documentation for more info");

        private readonly GUIContent CShaderPerfMode = new GUIContent("Blend Quality",
            "Can be used to reduce the number of textures blended per pixel to increase speed. May create blending artifacts when set too low");

        private readonly GUIContent CUseCustomSplatMaps = new GUIContent("Use Custom Splatmaps",
            "Use user provided splat maps instead of the ones unity generates");

        private readonly GUIContent CUVMode = new GUIContent("UV Mode", "Mode for Splat UV coordinates");
        public DebugOutput debugOutput = DebugOutput.None;
        public bool disableHeightBlend;
        public bool emissiveArray;
        ...
        public bool useCustomSplatMaps;
        ...
        {
            string ret;
            if (sFeatureNames.TryGetValue(feature, out ret)) return ret;
            var fn = Enum.GetName(typeof(DefineFeature), feature);
        ...
        public override void WriteFunctions(StringBuilder sb)
        {
        }

        public override void DrawFeatureGUI(Material mat)
        {
            var isSurfaceShader = mat.IsKeywordEnabled("_MSRENDERLOOP_SURFACESHADER");
            ...
            if (isSurfaceShader) lightingMode = (LightingMode) EditorGUILayout.EnumPopup(CLightingMode, lightingMode);
        ...
        public override void DrawShaderGUI(MicroSplatShaderGUI shaderGUI, Material mat, MaterialEditor materialEditor,
            MaterialProperty[] props)
        {
            if (MicroSplatUtilities.DrawRollup("Splats"))
            {
                ...
                if (!disableHeightBlend)
                {
                    ...
                    contrastProp.floatValue =
                        EditorGUILayout.Slider(CInterpContrast, contrastProp.floatValue, 1.0f, 0.0001f);
                }
                ...
                if (!mat.IsKeywordEnabled("_TRIPLANAR"))
                {
                    ...
                    var uvScale = shaderGUI.FindProp("_UVScale", props).vectorValue;
                    var scl = new Vector2(uvScale.x, uvScale.y);
                    var offset = new Vector2(uvScale.z, uvScale.w);
        ...
        public override string[] Pack()
        {
            ...
            if (perTexUVScale && samplerMode == SamplerMode.Default) samplerMode = SamplerMode.GradientSampler;
            ...
            if (emissiveArray) features.Add(GetFeatureName(DefineFeature._USEEMISSIVEMETAL));

            if (perfMode == PerformanceMode.Balanced)
                features.Add(GetFeatureName(DefineFeature._MAX3LAYER));
            else if (perfMode == PerformanceMode.Fastest) features.Add(GetFeatureName(DefineFeature._MAX2LAYER));
            if (disableHeightBlend) features.Add(GetFeatureName(DefineFeature._DISABLEHEIGHTBLENDING));
            ...
            else if (lightingMode == LightingMode.Lambert) features.Add(GetFeatureName(DefineFeature._BDRFLAMBERT));

            if (perTexUVScale) features.Add(GetFeatureName(DefineFeature._PERTEXUVSCALEOFFSET));

            if (perTexHeightOffset) features.Add(GetFeatureName(DefineFeature._PERTEXHEIGHTOFFSET));
            if (perTexHeightContrast) features.Add(GetFeatureName(DefineFeature._PERTEXHEIGHTCONTRAST));

            if (uvMode == UVMode.WorldSpace) features.Add(GetFeatureName(DefineFeature._WORLDUV));

            if (perTexInterpContrast) features.Add(GetFeatureName(DefineFeature._PERTEXINTERPCONTRAST));
            if (perTexTint) features.Add(GetFeatureName(DefineFeature._PERTEXTINT));
            if (perTexBrightness) features.Add(GetFeatureName(DefineFeature._PERTEXBRIGHTNESS));
            if (perTexContrast) features.Add(GetFeatureName(DefineFeature._PERTEXCONTRAST));
            if (perTexAOStr) features.Add(GetFeatureName(DefineFeature._PERTEXAOSTR));
            if (perTexNormStr) features.Add(GetFeatureName(DefineFeature._PERTEXNORMSTR));
            if (perTexSmoothStr) features.Add(GetFeatureName(DefineFeature._PERTEXSMOOTHSTR));
            if (perTexMetallic) features.Add(GetFeatureName(DefineFeature._PERTEXMETALLIC));
        ...
        public override void Unpack(string[] keywords)
        {
            ...
            packMode = HasFeature(keywords, DefineFeature._PACKINGHQ)
                ? TextureArrayConfig.PackingMode.Quality
                : TextureArrayConfig.PackingMode.Fastest;
            ...
            else if (HasFeature(keywords, DefineFeature._USEGRADMIP)) samplerMode = SamplerMode.GradientSampler;
            ...
            else if (HasFeature(keywords, DefineFeature._BDRFLAMBERT)) lightingMode = LightingMode.Lambert;
            ...
            if (HasFeature(keywords, DefineFeature._FORCEMODEL46)) shaderModel = ShaderModel.Force46;
            if (HasFeature(keywords, DefineFeature._FORCEMODEL50)) shaderModel = ShaderModel.Force50;
            ...
            else if (HasFeature(keywords, DefineFeature._DEBUG_OUTPUT_EMISSION)) debugOutput = DebugOutput.Emission;
        }
        ...
        public override void ComputeSampleCounts(string[] features, ref int arraySampleCount,
            ref int textureSampleCount, ref int maxSamples, ref int tessellationSamples, ref int depTexReadLevel)
        {
            textureSampleCount += (int) maxTextureCount / 4; // control textures
            if (perfMode == PerformanceMode.BestQuality)
            {
                arraySampleCount += 8;
                if (emissiveArray) arraySampleCount += 4;
                if (packMode == TextureArrayConfig.PackingMode.Quality) arraySampleCount += 4;
            }
            else if (perfMode == PerformanceMode.Balanced)
            {
                arraySampleCount += 6;
                if (emissiveArray) arraySampleCount += 3;
                if (packMode == TextureArrayConfig.PackingMode.Quality) arraySampleCount += 3;
            }
            else if (perfMode == PerformanceMode.Fastest)
            {
                arraySampleCount += 4;
                if (emissiveArray) arraySampleCount += 2;
                if (packMode == TextureArrayConfig.PackingMode.Quality) arraySampleCount += 2;
            }
        }
        ...
        public override void DrawPerTextureGUI(int index, Material mat, MicroSplatPropData propData)
        {
            ...
            if (perTexUVScale && samplerMode == SamplerMode.Default)
                EditorGUILayout.HelpBox(
                    "On some GPUs, small artifacts can appear with per-texture UV scales. Switching sampler mode to Mip (fast) or Gradient (slow) will fix the issue",
                    MessageType.Info);
            ...
            if (!disableHeightBlend)
                perTexInterpContrast = DrawPerTexFloatSlider(index, 1,
                    GetFeatureName(DefineFeature._PERTEXINTERPCONTRAST),
