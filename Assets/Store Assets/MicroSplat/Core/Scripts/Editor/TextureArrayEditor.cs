...
namespace JBooth.MicroSplat.Utility
{
    ...
    public class TextureArrayEditor : Editor
    {
        ...
        {
            var texture2DArray = (Texture2DArray) target;
            for (var i = 0; i <= texture2DArray.depth - 1; i++)
            {
                // annoying to work around all the odd unity issues
                // copy to temp texture..
                var tempTexture = new Texture2D(texture2DArray.width, texture2DArray.height, texture2DArray.format,
                    true, true);
        ...
        {
            for (var i = 0; i <= _previewTextureList.Count - 1; i++) DestroyImmediate(_previewTextureList[i]);
        ...
        public override void OnInspectorGUI()
        {
            ...
            var texture2DArray = (Texture2DArray) target;
            var imageButtons = new GUIContent[texture2DArray.depth];
            ...
            var imageWidth = 120;
            var columns = Mathf.FloorToInt(EditorGUIUtility.currentViewWidth / imageWidth);
            var rows = Mathf.CeilToInt((float) imageButtons.Length / columns);
            var gridHeight = rows * imageWidth;
            GUILayout.SelectionGrid(0, imageButtons, columns, GUI.skin.label, GUILayout.MaxWidth(columns * imageWidth),
                GUILayout.MaxHeight(gridHeight));

            texture2DArray.anisoLevel = EditorGUILayout.IntField("Anisotropic", texture2DArray.anisoLevel);
            texture2DArray.filterMode =
                (FilterMode) EditorGUILayout.EnumPopup("Filter Mode", texture2DArray.filterMode);
