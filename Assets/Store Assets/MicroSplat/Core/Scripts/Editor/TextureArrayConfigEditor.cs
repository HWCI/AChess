...
namespace JBooth.MicroSplat
{
    ...
    {
        private static readonly GUIContent CPlatformOverrides = new GUIContent("Platform Compression Overrides",
            "Override the compression type on a per platform basis");

        private static readonly GUIContent CTextureMode = new GUIContent("Texturing Mode",
            "Do you have just diffuse and normal, or a fully PBR pipeline with height, smoothness, and ao textures?");

        private static readonly GUIContent CSourceTextureSize =
            new GUIContent("Source Texture Size", "Reduce source texture size to save memory in builds");

        private static readonly GUIContent CPackingMode = new GUIContent("Packing Mode",
            "Pack textures into two arrays (fastest) or three (best quality)");
#if __MICROSPLAT_TEXTURECLUSTERS__
      static GUIContent CClusterMode =
 new GUIContent("Cluster Mode", "Add extra slots for packing parallel arrays for texture clustering");
#endif
#if __MICROSPLAT_DETAILRESAMPLE__
      static GUIContent CAntiTileArray =
 new GUIContent("AntiTile Array", "Create an array for each texture to have it's own Noise Normal, Detail, and Distance noise texture");
#endif

        private static readonly GUIContent CEmisMetalArray = new GUIContent("Emissive/Metal array",
            "Create a texture array for emissive and metallic materials");

        private static readonly GUIContent CDiffuse = new GUIContent("Diffuse", "Diffuse or Albedo texture");
        private static readonly GUIContent CNormal = new GUIContent("Normal", "Normal map");
        private static readonly GUIContent CAO = new GUIContent("AO", "Ambient Occlusion map");

        private static readonly GUIContent CSmoothness =
            new GUIContent("Smoothness", "Smoothness map, or roughness map with invert on");

        private static readonly GUIContent CHeight = new GUIContent("Height", "Height Map");
        private static readonly GUIContent CAlpha = new GUIContent("Alpha", "Alpha Map");

        private static readonly GUIContent CNoiseNormal =
            new GUIContent("Noise Normal", "Normal to bend in over a larger area");

        private static readonly GUIContent CDetailNoise =
            new GUIContent("Detail", "Noise texture to blend in when close");

        private static readonly GUIContent CDistanceNoise =
            new GUIContent("Distance", "Noise texture to blend in when far away");

        private void DrawHeader(TextureArrayConfig cfg)
        {
            if (cfg.textureMode != TextureArrayConfig.TextureMode.Basic)
            {
                ...
                cfg.allTextureChannelHeight =
                    (TextureArrayConfig.AllTextureChannel) EditorGUILayout.EnumPopup(cfg.allTextureChannelHeight,
                        GUILayout.Width(64));
                EditorGUILayout.EndVertical();

                EditorGUILayout.BeginVertical();
                EditorGUILayout.LabelField(CSmoothness, GUILayout.Width(64));
                cfg.allTextureChannelSmoothness =
                    (TextureArrayConfig.AllTextureChannel) EditorGUILayout.EnumPopup(cfg.allTextureChannelSmoothness,
                        GUILayout.Width(64));
                EditorGUILayout.EndVertical();
                ...
                if (cfg.IsAdvancedDetail())
                {
                    ...
                    cfg.allTextureChannelAlpha =
                        (TextureArrayConfig.AllTextureChannel) EditorGUILayout.EnumPopup(cfg.allTextureChannelAlpha,
                            GUILayout.Width(64));
                }
                else
                {
                    EditorGUILayout.LabelField(CAO, GUILayout.Width(64));
                    cfg.allTextureChannelAO =
                        (TextureArrayConfig.AllTextureChannel) EditorGUILayout.EnumPopup(cfg.allTextureChannelAO,
                            GUILayout.Width(64));
                }
        ...
        private void DrawAntiTileEntry(TextureArrayConfig cfg, TextureArrayConfig.TextureEntry e, int i)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            e.noiseNormal = (Texture2D) EditorGUILayout.ObjectField(e.noiseNormal, typeof(Texture2D), false,
                GUILayout.Width(64), GUILayout.Height(64));
            EditorGUILayout.EndVertical();

            EditorGUILayout.BeginVertical();
            EditorGUILayout.LabelField(CDetailNoise, GUILayout.Width(92));
            e.detailNoise = (Texture2D) EditorGUILayout.ObjectField(e.detailNoise, typeof(Texture2D), false,
                GUILayout.Width(64), GUILayout.Height(64));
            e.detailChannel =
                (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.detailChannel, GUILayout.Width(64));
            EditorGUILayout.EndVertical();

            EditorGUILayout.BeginVertical();
            EditorGUILayout.LabelField(CDistanceNoise, GUILayout.Width(92));
            e.distanceNoise = (Texture2D) EditorGUILayout.ObjectField(e.distanceNoise, typeof(Texture2D), false,
                GUILayout.Width(64), GUILayout.Height(64));
            e.distanceChannel =
                (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.distanceChannel, GUILayout.Width(64));
            EditorGUILayout.EndVertical();
            ...
            {
                var index = (int) Mathf.Repeat(i, 3);
                e.noiseNormal = MicroSplatUtilities.GetAutoTexture("microsplat_def_detail_normal_0" + (index + 1));
            }
        }

        private void SwapEntry(TextureArrayConfig cfg, int src, int targ)
        {
        ...
        private bool DrawTextureEntry(TextureArrayConfig cfg, TextureArrayConfig.TextureEntry e, int i,
            bool controls = true)
        {
            var ret = false;
            if (controls)
            {
                ...
                else
                {
                    EditorGUILayout.HelpBox(
                        "Removing an entry completely can cause texture choices to change on existing terrains. You can leave it blank to preserve the texture order and MicroSplat will put a dummy texture into the array.",
                        MessageType.Warning);
                    ret = GUILayout.Button("Delete Entry");
                }

                if (GUILayout.Button("Up", GUILayout.Width(40))) SwapEntry(cfg, i, i - 1);
                if (GUILayout.Button("Down", GUILayout.Width(40))) SwapEntry(cfg, i, i + 1);
                EditorGUILayout.EndHorizontal();
            }
            ...
            e.substance =
 (ProceduralMaterial)EditorGUILayout.ObjectField(e.substance, typeof(ProceduralMaterial), false, GUILayout.Width(64), GUILayout.Height(64));
            EditorGUILayout.EndVertical();
#endif
            }

            EditorGUILayout.BeginVertical();
            if (controls) EditorGUILayout.LabelField(CDiffuse, GUILayout.Width(64));
            e.diffuse = (Texture2D) EditorGUILayout.ObjectField(e.diffuse, typeof(Texture2D), false,
                GUILayout.Width(64), GUILayout.Height(64));
            EditorGUILayout.EndVertical();

            EditorGUILayout.BeginVertical();
            if (controls) EditorGUILayout.LabelField(CNormal, GUILayout.Width(64));
            e.normal = (Texture2D) EditorGUILayout.ObjectField(e.normal, typeof(Texture2D), false, GUILayout.Width(64),
                GUILayout.Height(64));
            EditorGUILayout.EndVertical();
            ...
            {
                EditorGUILayout.BeginVertical();
                if (controls) EditorGUILayout.LabelField(CHeight, GUILayout.Width(64));
                e.height = (Texture2D) EditorGUILayout.ObjectField(e.height, typeof(Texture2D), false,
                    GUILayout.Width(64), GUILayout.Height(64));
                if (cfg.allTextureChannelHeight == TextureArrayConfig.AllTextureChannel.Custom)
                    e.heightChannel =
                        (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.heightChannel,
                            GUILayout.Width(64));
                EditorGUILayout.EndVertical();

                EditorGUILayout.BeginVertical();
                if (controls) EditorGUILayout.LabelField(CSmoothness, GUILayout.Width(64));
                e.smoothness = (Texture2D) EditorGUILayout.ObjectField(e.smoothness, typeof(Texture2D), false,
                    GUILayout.Width(64), GUILayout.Height(64));
                if (cfg.allTextureChannelSmoothness == TextureArrayConfig.AllTextureChannel.Custom)
                    e.smoothnessChannel =
                        (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.smoothnessChannel,
                            GUILayout.Width(64));
                EditorGUILayout.BeginHorizontal();
                ...
                if (cfg.IsAdvancedDetail())
                {
                    if (controls) EditorGUILayout.LabelField(CAlpha, GUILayout.Width(64));
                    e.alpha = (Texture2D) EditorGUILayout.ObjectField(e.alpha, typeof(Texture2D), false,
                        GUILayout.Width(64), GUILayout.Height(64));
                    if (cfg.allTextureChannelAlpha == TextureArrayConfig.AllTextureChannel.Custom)
                        e.alphaChannel =
                            (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.alphaChannel,
                                GUILayout.Width(64));
                    EditorGUILayout.BeginHorizontal();
                ...
                {
                    if (controls) EditorGUILayout.LabelField(CAO, GUILayout.Width(64));
                    e.ao = (Texture2D) EditorGUILayout.ObjectField(e.ao, typeof(Texture2D), false, GUILayout.Width(64),
                        GUILayout.Height(64));
                    if (cfg.allTextureChannelAO == TextureArrayConfig.AllTextureChannel.Custom)
                        e.aoChannel =
                            (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.aoChannel,
                                GUILayout.Width(64));
                }
                ...
                {
                    EditorGUILayout.BeginVertical();
                    if (controls) EditorGUILayout.LabelField(new GUIContent("Emissive"), GUILayout.Width(64));
                    e.emis = (Texture2D) EditorGUILayout.ObjectField(e.emis, typeof(Texture2D), false,
                        GUILayout.Width(64), GUILayout.Height(64));
                    EditorGUILayout.EndVertical();

                    EditorGUILayout.BeginVertical();
                    if (controls) EditorGUILayout.LabelField(new GUIContent("Metal"), GUILayout.Width(64));
                    e.metal = (Texture2D) EditorGUILayout.ObjectField(e.metal, typeof(Texture2D), false,
                        GUILayout.Width(64), GUILayout.Height(64));
                    e.metalChannel =
                        (TextureArrayConfig.TextureChannel) EditorGUILayout.EnumPopup(e.metalChannel,
                            GUILayout.Width(64));
        ...
        public static bool GetFromTerrain(TextureArrayConfig cfg, Terrain t)
        {
            if (t != null && cfg.sourceTextures.Count == 0 && t.terrainData != null)
            {
                ...
                var count = t.terrainData.terrainLayers.Length;
        ...
        private static void GetFromTerrain(TextureArrayConfig cfg)
        {
        ...
        {
            var configPath = AssetDatabase.GenerateUniqueAssetPath(path + "/MicroSplatConfig.asset");
            var cfg = CreateInstance<TextureArrayConfig>();
        ...
        private void Remove(TextureArrayConfig cfg, int i)
        {
        ...
        private void Reset(TextureArrayConfig cfg, int i)
        {
        ...
        private static void MatchArrayLength(TextureArrayConfig cfg)
        {
            var srcCount = cfg.sourceTextures.Count;
            var change = false;
            ...
            if (change) EditorUtility.SetDirty(cfg);
        }

        private void DrawOverrideGUI(TextureArrayConfig cfg)
        {
        ...
        public override void OnInspectorGUI()
        {
            ...
            cfg.sourceTextureSize =
                (TextureArrayConfig.SourceTextureSize) EditorGUILayout.EnumPopup(CSourceTextureSize,
                    cfg.sourceTextureSize);
            ...
            {
               cfg.clusterMode =
 (TextureArrayConfig.ClusterMode)EditorGUILayout.EnumPopup(CClusterMode, cfg.clusterMode);
            }
#endif
            }
            ...
            if (root.isExpanded)
            {
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Select PBR mode to provide custom height, smoothness, and ao textures to greatly increase quality!",
                        MessageType.Info);
                }
            ...
            {
                EditorGUILayout.HelpBox(
                    "Don't have a normal map? Any missing textures will be generated automatically from the best available source texture",
                    MessageType.Info);
                var disableClusters = cfg.IsAdvancedDetail();
                DrawHeader(cfg);
                for (var i = 0; i < cfg.sourceTextures.Count; ++i)
                    using (new GUILayout.VerticalScope(GUI.skin.box))
                    {
                        var remove = DrawTextureEntry(cfg, cfg.sourceTextures[i], i);
                        ...
                        if (cfg.antiTileArray) DrawAntiTileEntry(cfg, cfg.sourceTextures[i], i);
            ...
            if (EditorGUI.EndChangeCheck()) EditorUtility.SetDirty(cfg);
        }

        private static bool IsLinear(TextureImporter ti)
        {
            return ti.sRGBTexture == false;
        }

        private static Texture2D ResizeTexture(Texture2D source, int width, int height, bool linear)
        {
            ...
            var ret = new Texture2D(width, height, TextureFormat.ARGB32, true, linear);
        ...
        public static TextureFormat GetTextureFormat(TextureArrayConfig cfg, TextureArrayConfig.Compression cmp)
        {
            ...
            if (cmp == TextureArrayConfig.Compression.ForcePVR)
                return TextureFormat.PVRTC_RGBA4;
            if (cmp == TextureArrayConfig.Compression.ForceASTC)
                return TextureFormat.ASTC_RGBA_4x4;
            if (cmp == TextureArrayConfig.Compression.ForceDXT)
                return TextureFormat.DXT5;
            if (cmp == TextureArrayConfig.Compression.ForceCrunch)
                return TextureFormat.DXT5Crunched;
            if (cmp == TextureArrayConfig.Compression.Uncompressed) return TextureFormat.ARGB32;
        ...
        private static Texture2D RenderMissingTexture(Texture2D src, string shaderPath, int width, int height,
            int channel = -1)
        {
            var res = new Texture2D(width, height, TextureFormat.ARGB32, true, true);
            var resRT = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
            ...
            var genMat = new Material(Shader.Find(shaderPath));
            if (channel >= 0) genMat.SetInt("_Channel", channel);
        ...
        private static void MergeInChannel(Texture2D target, int targetChannel,
            Texture2D merge, int mergeChannel, bool linear, bool invert = false)
        {
            var src = ResizeTexture(merge, target.width, target.height, linear);
            var sc = src.GetPixels();
            var tc = target.GetPixels();
        ...
        private static Texture2D NormalizeAlphaMask(Texture2D src, int targetChannel)
        {
            var result = RenderMissingTexture(src, "Unlit/Texture", src.width, src.height);
...
#if !UNITY_2017_3_OR_NEWER
      static Texture2D BakeSubstance(string path, ProceduralTexture pt, bool linear = true, bool isNormal =
 false, bool invert = false)
      ...
      static void PreprocessTextureEntries(TextureArrayConfig cfg)
      {
        ...
        private static TextureArrayConfig staticConfig;

        private void DelayedCompileConfig()
        {
            CompileConfig(staticConfig);
        }

        private static string GetDiffPath(TextureArrayConfig cfg, string ext)
        {
        ...
        private static string GetNormPath(TextureArrayConfig cfg, string ext)
        {
            var path = AssetDatabase.GetAssetPath(cfg);
            // create array path
            path = path.Replace("\\", "/");
        ...
        private static string GetSmoothAOPath(TextureArrayConfig cfg, string ext)
        {
            var path = AssetDatabase.GetAssetPath(cfg);
            // create array path
            path = path.Replace("\\", "/");
        ...
        private static string GetAntiTilePath(TextureArrayConfig cfg, string ext)
        {
            var path = AssetDatabase.GetAssetPath(cfg);
            // create array path
            path = path.Replace("\\", "/");
        ...
        private static string GetEmisPath(TextureArrayConfig cfg, string ext)
        {
            var path = AssetDatabase.GetAssetPath(cfg);
            // create array path
            path = path.Replace("\\", "/");
        ...
        private static int SizeToMipCount(int size)
        {
            var mips = 11;
            if (size == 4096)
                mips = 13;
        ...
        private static void ShrinkSourceTexture(Texture2D tex, TextureArrayConfig.SourceTextureSize stz)
        {
            if (tex != null)
            {
        ...
        private static void ShrinkSourceTextures(List<TextureArrayConfig.TextureEntry> textures,
            TextureArrayConfig.SourceTextureSize stz)
        {
            if (textures == null)
                return;
        ...
        private static void RestoreSourceTexture(Texture2D tex)
        {
            if (tex != null)
            {
                var ai = AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(tex));
        ...
        private static void RestoreSourceTextures(List<TextureArrayConfig.TextureEntry> textures,
            TextureArrayConfig.SourceTextureSize stz)
        {
            if (textures == null)
                return;
        ...
        private static void CompileConfig(TextureArrayConfig cfg,
            ...
            bool isCluster = false)
        {
            ...
            var diffuseIsLinear = false;

            var settings = GetSettingsGroup(cfg, EditorUserBuildSettings.activeBuildTarget);


            var diffuseWidth = (int) settings.diffuseSettings.textureSize;
            var diffuseHeight = (int) settings.diffuseSettings.textureSize;
            var normalWidth = (int) settings.normalSettings.textureSize;
            var normalHeight = (int) settings.normalSettings.textureSize;
            var antiTileWidth = (int) settings.antiTileSettings.textureSize;
            var antiTileHeight = (int) settings.antiTileSettings.textureSize;
            var emisWidth = (int) settings.emissiveSettings.textureSize;
            var emisHeight = (int) settings.emissiveSettings.textureSize;

            var diffuseAnisoLevel = settings.diffuseSettings.Aniso;
            var normalAnisoLevel = settings.normalSettings.Aniso;
            var antiTileAnisoLevel = settings.antiTileSettings.Aniso;
            var emisAnisoLevel = settings.emissiveSettings.Aniso;

            var diffuseFilter = settings.diffuseSettings.filterMode;
            var normalFilter = settings.normalSettings.filterMode;
            var antiTileFilter = settings.antiTileSettings.filterMode;
            var emisFilter = settings.emissiveSettings.filterMode;

            var diffuseMipCount = SizeToMipCount(diffuseWidth);
            var normalMipCount = SizeToMipCount(normalWidth);
            var smoothAOMipCount = SizeToMipCount((int) settings.smoothSettings.textureSize);
            var antiTileMipCount = SizeToMipCount(antiTileWidth);
            var emisMipCount = SizeToMipCount(emisWidth);

            var texCount = src.Count;
            if (texCount < 1)
                texCount = 1;

            // diffuse
            var diffuseArray = new Texture2DArray(diffuseWidth, diffuseHeight, texCount,
            ...
            var normalSAOArray = new Texture2DArray(normalWidth, normalHeight, texCount, nmlcomp, true, true);
            ...
            if (cfg.packingMode == TextureArrayConfig.PackingMode.Quality)
                smoothAOArray = new Texture2DArray((int) settings.smoothSettings.textureSize,
                    (int) settings.smoothSettings.textureSize, texCount,
            ...
            for (var i = 0; i < src.Count; ++i)
                try
                {
                    EditorUtility.DisplayProgressBar("Packing textures...", "", i / (float) src.Count);
                    ...
                    var e = src[i];
                    var diffuse = e.diffuse;
                    if (diffuse == null) diffuse = Texture2D.whiteTexture;

                    // resulting maps
                    var diffuseHeightTex = ResizeTexture(diffuse, diffuseWidth, diffuseHeight, diffuseIsLinear);
                    Texture2D normalSAOTex = null;
                    Texture2D smoothAOTex = null;
                    Texture2D antiTileTex = null;
                    Texture2D emisTex = null;

                    var heightChannel = (int) e.heightChannel;
                    var aoChannel = (int) e.aoChannel;
                    var smoothChannel = (int) e.smoothnessChannel;
                    var alphaChannel = (int) e.alphaChannel;
                    var detailChannel = (int) e.detailChannel;
                    var distanceChannel = (int) e.distanceChannel;
                    var metalChannel = (int) e.metalChannel;
                    ...
                    {
                        if (e.height == null)
                            normalSAOTex = RenderMissingTexture(diffuse, "Hidden/MicroSplat/NormalSAOFromDiffuse",
                                normalWidth, normalHeight);
                        else
                            normalSAOTex = RenderMissingTexture(e.height, "Hidden/MicroSplat/NormalSAOFromHeight",
                                normalWidth, normalHeight, heightChannel);
                    }
                    else
                    {
                        // copy, but go ahead and generate other channels in case they aren't provided later.
                        normalSAOTex = RenderMissingTexture(e.normal, "Hidden/MicroSplat/NormalSAOFromNormal",
                            normalWidth, normalHeight);
                    }

                    if (!isCluster && cfg.antiTileArray)
                        antiTileTex = RenderMissingTexture(e.noiseNormal, "Hidden/MicroSplat/NormalSAOFromNormal",
                            antiTileWidth, antiTileHeight);

                    var destroyHeight = false;
                    var height = e.height;
                    if (height == null)
                    {
                        destroyHeight = true;
                        height = RenderMissingTexture(normalSAOTex, "Hidden/MicroSplat/HeightFromNormal", diffuseHeight,
                            diffuseWidth);
                    }

                    MergeInChannel(diffuseHeightTex, (int) TextureArrayConfig.TextureChannel.A, height, heightChannel,
                        diffuseIsLinear);


                    if (cfg.emisMetalArray && !cfg.IsAdvancedDetail())
                    {
                        emisTex = ResizeTexture(e.emis != null ? e.emis : Texture2D.blackTexture, emisWidth, emisHeight,
                            diffuseIsLinear);
                        var metal = ResizeTexture(e.metal != null ? e.metal : Texture2D.blackTexture, emisWidth,
                            emisHeight, true);
                    ...
                    if (cfg.IsAdvancedDetail())
                    {
                        if (e.alpha != null)
                        {
                            var alpha = e.normalizeAlpha
                                ? NormalizeAlphaMask(e.alpha, (int) TextureArrayConfig.TextureChannel.B)
                                : e.alpha;
                            MergeInChannel(normalSAOTex, (int) TextureArrayConfig.TextureChannel.B, alpha, alphaChannel,
                                true);
                        }
                        else
                        {
                            MergeInChannel(normalSAOTex, (int) TextureArrayConfig.TextureChannel.B,
                                Texture2D.whiteTexture, 0, true);
                        }
                    }
                    else if (e.ao != null)
                    {
                    ...
                    if (e.smoothness != null)
                        MergeInChannel(normalSAOTex, (int) TextureArrayConfig.TextureChannel.R, e.smoothness,
                            smoothChannel, true, e.isRoughness);

                    if (cfg.packingMode == TextureArrayConfig.PackingMode.Quality)
                    {
                        // clear non-normal data to help compression quality
                        var cols = normalSAOTex.GetPixels();
                        ...
                        {
                            if (e.height == null)
                                smoothAOTex = RenderMissingTexture(diffuse, "Hidden/MicroSplat/NormalSAOFromDiffuse",
                                    normalWidth, normalHeight);
                            else
                                smoothAOTex = RenderMissingTexture(e.height, "Hidden/MicroSplat/NormalSAOFromHeight",
                                    normalWidth, normalHeight, heightChannel);
                        }
                        else
                        {
                            // copy, but go ahead and generate other channels in case they aren't provided later.
                            smoothAOTex = RenderMissingTexture(e.normal, "Hidden/MicroSplat/NormalSAOFromNormal",
                                normalWidth, normalHeight);
                        }
                        ...
                        if (e.ao != null)
                            MergeInChannel(smoothAOTex, (int) TextureArrayConfig.TextureChannel.A, e.ao, aoChannel,
                                true);

                        if (e.smoothness != null)
                            MergeInChannel(smoothAOTex, (int) TextureArrayConfig.TextureChannel.G, e.smoothness,
                                smoothChannel, true, e.isRoughness);
                    }
                    ...
                    {
                        var detail = e.detailNoise;
                        var distance = e.distanceNoise;
                        var destroyDetail = false;
                        var destroyDistance = false;
                        if (detail == null)
                        {
                        ...
                        MergeInChannel(antiTileTex, (int) TextureArrayConfig.TextureChannel.R, detail, detailChannel,
                            true, false);
                        MergeInChannel(antiTileTex, (int) TextureArrayConfig.TextureChannel.B, distance,
                            distanceChannel, true, false);

                        if (destroyDetail) DestroyImmediate(detail);
                        if (destroyDistance) DestroyImmediate(distance);
                    }

#if UNITY_2018_3_OR_NEWER
                    var tq = (int) TextureCompressionQuality.Normal;
#else
               var tq = UnityEngine.TextureCompressionQuality.Normal;
                    ...
                    if (settings.normalSettings.compression != TextureArrayConfig.Compression.Uncompressed)
                        EditorUtility.CompressTexture(normalSAOTex,
                            GetTextureFormat(cfg, settings.normalSettings.compression), tq);

                    if (settings.diffuseSettings.compression != TextureArrayConfig.Compression.Uncompressed)
                        EditorUtility.CompressTexture(diffuseHeightTex,
                            GetTextureFormat(cfg, settings.diffuseSettings.compression), tq);

                    if (smoothAOTex != null && cfg.packingMode == TextureArrayConfig.PackingMode.Quality)
                        EditorUtility.CompressTexture(smoothAOTex,
                            GetTextureFormat(cfg, settings.smoothSettings.compression), tq);

                    if (antiTileTex != null && settings.antiTileSettings.compression !=
                        TextureArrayConfig.Compression.Uncompressed)
                        EditorUtility.CompressTexture(antiTileTex,
                            GetTextureFormat(cfg, settings.antiTileSettings.compression), tq);

                    if (cfg.emisMetalArray && !cfg.IsAdvancedDetail() && emisTex != null &&
                        settings.emissiveSettings.compression != TextureArrayConfig.Compression.Uncompressed)
                        EditorUtility.CompressTexture(emisTex,
                            GetTextureFormat(cfg, settings.emissiveSettings.compression), tq);
            ...
            if (antiTileArray != null) antiTileArray.Apply(false, true);
            if (emisArray != null) emisArray.Apply(false, true);
            if (smoothAOArray != null) smoothAOArray.Apply(false, true);

            var diffPath = GetDiffPath(cfg, ext);
            var normSAOPath = GetNormPath(cfg, ext);
            var antiTilePath = GetAntiTilePath(cfg, ext);
            var emisPath = GetEmisPath(cfg, ext);
            var smoothAOPath = GetSmoothAOPath(cfg, ext);
