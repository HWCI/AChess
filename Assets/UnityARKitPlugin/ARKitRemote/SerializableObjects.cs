...
namespace UnityEngine.XR.iOS.Utils
{
    ...
    public class serializableUnityARCamera
    {
        ...
        public static implicit operator serializableUnityARCamera(UnityARCamera rValue)
        {
            return new serializableUnityARCamera(rValue.worldTransform, rValue.projectionMatrix, rValue.trackingState,
                ...
                rValue.pointCloudData, rValue.worldMappingStatus);
        }
        ...
        public static implicit operator UnityARCamera(serializableUnityARCamera rValue)
        {
            return new UnityARCamera(rValue.worldTransform, rValue.projectionMatrix, rValue.trackingState,
                rValue.trackingReason, rValue.videoParams, rValue.lightData, rValue.displayTransform, rValue.pointCloud,
                rValue.worldMappingStatus);
        }
    }
    ...
    public class serializablePlaneGeometry
    {
        ...
        public serializablePlaneGeometry(byte[] inputVertices, byte[] inputTexCoords, byte[] inputTriIndices,
            byte[] boundaryVerts)
        {
        ...
        public Vector3[] Vertices
        {
            get
            {
                if (vertices != null)
                {
                    var numVectors = vertices.Length / (3 * sizeof(float));
                    var verticesVec = new Vector3[numVectors];
        ...
        public Vector3[] BoundaryVertices
        {
            get
            {
                ...
                {
                    var numVectors = boundaryVertices.Length / (3 * sizeof(float));
                    var verticesVec = new Vector3[numVectors];
                    for (var i = 0; i < numVectors; i++)
                    {
                        var bufferStart = i * 3;
        ...
        public Vector2[] TexCoords
        {
            get
            {
                if (texCoords != null)
                {
                    var numVectors = texCoords.Length / (2 * sizeof(float));
                    var texCoordVec = new Vector2[numVectors];
                    for (var i = 0; i < numVectors; i++)
                    {
                        var bufferStart = i * 2;
        ...
        public int[] TriangleIndices
        {
            get
            {
                if (triIndices != null)
                {
                    var triIndexVec = triIndices.Deserialize<int[]>();
                    return triIndexVec;
                }
    ...
    public class serializableUnityARFaceAnchor
    {
        ...
        public serializableUnityARFaceAnchor(serializableUnityARMatrix4x4 wt, serializableFaceGeometry fg,
            Dictionary<string, float> bs, byte[] idstr, bool bIsTracked)
        {
            worldTransform = wt;
            faceGeometry = fg;
            arBlendShapes = bs;
    ...
    public class serializableARSessionConfiguration
    {
        ...
        public serializableARSessionConfiguration(UnityARAlignment align, UnityARPlaneDetection planeDet,
            bool getPtCloud, bool enableLightEst, bool enableAutoFoc)
        {
            alignment = align;
        ...
        public static implicit operator serializableARSessionConfiguration(ARKitWorldTrackingSessionConfiguration awtsc)
        {
            return new serializableARSessionConfiguration(awtsc.alignment, awtsc.planeDetection,
                awtsc.getPointCloudData, awtsc.enableLightEstimation, awtsc.enableAutoFocus);
        }
        ...
        public static implicit operator ARKitWorldTrackingSessionConfiguration(serializableARSessionConfiguration sasc)
        {
            return new ARKitWorldTrackingSessionConfiguration(sasc.alignment, sasc.planeDetection,
                sasc.getPointCloudData, sasc.enableLightEstimation, sasc.enableAutoFocus);
        }
