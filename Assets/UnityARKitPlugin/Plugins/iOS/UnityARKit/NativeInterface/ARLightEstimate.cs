...
namespace UnityEngine.XR.iOS
{
    ...
    public struct MarshalDirectionalLightEstimate
    {
        ...
        public float[] SphericalHarmonicCoefficients
        {
            get { return MarshalCoefficients(sphericalHarmonicCoefficientsPtr); }
        }

        private float[] MarshalCoefficients(IntPtr ptrFloats)
        {
            var numFloats = 27;
            var workCoefficients = new float[numFloats];
            Marshal.Copy(ptrFloats, workCoefficients, 0, numFloats);
    ...
    public struct UnityMarshalLightData
    {
        ...
        public static implicit operator UnityARLightData(UnityMarshalLightData rValue)
        {
            ...
            {
                var lightDirAndIntensity = rValue.arDirectonalLightEstimate.primaryDirAndIntensity;
                var lightDir = new Vector3(lightDirAndIntensity.x, lightDirAndIntensity.y, lightDirAndIntensity.z);
                var shc = rValue.arDirectonalLightEstimate.SphericalHarmonicCoefficients;
