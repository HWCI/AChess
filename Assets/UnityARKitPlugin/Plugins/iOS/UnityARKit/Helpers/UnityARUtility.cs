namespace UnityEngine.XR.iOS
{
    public class UnityARUtility
    {
        ...
        public static GameObject CreatePlaneInScene(ARPlaneAnchor arPlaneAnchor)
        {
            GameObject plane;
            if (planePrefab != null)
                plane = Object.Instantiate(planePrefab);
            else
            ...
            var apmr = plane.GetComponent<ARKitPlaneMeshRender>();
            if (apmr != null) apmr.InitiliazeMesh(arPlaneAnchor);
        ...
        public static GameObject UpdatePlaneWithAnchorTransform(GameObject plane, ARPlaneAnchor arPlaneAnchor)
        {
            ...
            var apmr = plane.GetComponent<ARKitPlaneMeshRender>();
            if (apmr != null) apmr.UpdateMesh(arPlaneAnchor);


            var mf = plane.GetComponentInChildren<MeshFilter>();

            if (mf != null)
                if (apmr == null)
                {
                    //since our plane mesh is actually 10mx10m in the world, we scale it here by 0.1f
                    mf.gameObject.transform.localScale = new Vector3(arPlaneAnchor.extent.x * 0.1f,
                        arPlaneAnchor.extent.y * 0.1f, arPlaneAnchor.extent.z * 0.1f);

                    //convert our center position to unity coords
                    mf.gameObject.transform.localPosition = new Vector3(arPlaneAnchor.center.x, arPlaneAnchor.center.y,
